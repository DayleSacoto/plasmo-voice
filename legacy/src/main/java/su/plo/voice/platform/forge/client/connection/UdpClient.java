...
public final class UdpClient implements AutoCloseable {
    ...
    private void diagnostics(DatagramSocket endpoint, long now, long keepAlive) {
        ...
        if (snapshots.due(now)) {
            DEBUG.log(Category.UDP, "snapshot: state={}, generation={}, local={}, remote={}, keepAliveAge={}ms, {}, thread={}",
                    stateName(), generation, endpoint.getLocalSocketAddress(), endpoint.getRemoteSocketAddress(),
                    now - keepAlive, stats.snapshot(now), VoiceDebug.thread());
        }
    }
